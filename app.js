...
// ── Category map (emoji + colour) ─────────
const CATEGORIES = {
  salary:       { label: 'Salary',       emoji: '💼', color: '#22c55e' },
  freelance:    { label: 'Freelance',    emoji: '🖥️', color: '#a3e635' },
  investment:   { label: 'Investment',   emoji: '📈', color: '#34d399' },
  gift:         { label: 'Gift',         emoji: '🎁', color: '#f472b6' },
  other_income: { label: 'Other Income', emoji: '💰', color: '#94a3b8' },
  food:         { label: 'Food',         emoji: '🍔', color: '#f97316' },
  transport:    { label: 'Transport',    emoji: '🚗', color: '#3b82f6' },
  housing:      { label: 'Housing',      emoji: '🏠', color: '#8b5cf6' },
  ...
  shopping:     { label: 'Shopping',     emoji: '🛍️', color: '#6366f1' },
  education:    { label: 'Education',    emoji: '📚', color: '#06b6d4' },
  savings:      { label: 'Savings',      emoji: '🏦', color: '#10b981' },
  other_expense:{ label: 'Other Expense',emoji: '📦', color: '#94a3b8' },
};
...
// ── State ──────────────────────────────────
let transactions = [];
let budgets      = {};   // { month: 'YYYY-MM', amount: number }[]
  ...
  el.innerHTML = list.map(t => {
    ...
    return `
      <div class="tx-item" data-id="${t.id}">
        <div class="tx-icon ${t.type}">${cat.emoji}</div>
        <div class="tx-body">
          <div class="tx-desc" title="${escHtml(t.description)}">${escHtml(t.description)}</div>
          <div class="tx-meta">
            <span class="cat-chip">${cat.label}</span>
            <span>${fmtDate(t.date)}</span>
            ${t.note ? `<span title="${escHtml(t.note)}">📝 ${escHtml(t.note.slice(0,30))}${t.note.length>30?'…':''}</span>` : ''}
          </div>
        </div>
        <span class="tx-amount ${t.type}">${sign}${fmt(t.amount)}</span>
        <div class="tx-actions">
          <button class="btn btn-outline btn-sm" onclick="startEdit('${t.id}')" title="Edit">✏️</button>
          <button class="btn btn-danger btn-sm"  onclick="deleteTx('${t.id}')"  title="Delete">🗑️</button>
        </div>
      </div>`;
  }).join('');
};
...
const renderCategoryOptions = () => {
  ...
  sel.innerHTML = cats.map(k =>
    `<option value="${k}">${CATEGORIES[k].emoji} ${CATEGORIES[k].label}</option>`
  ).join('');
};
